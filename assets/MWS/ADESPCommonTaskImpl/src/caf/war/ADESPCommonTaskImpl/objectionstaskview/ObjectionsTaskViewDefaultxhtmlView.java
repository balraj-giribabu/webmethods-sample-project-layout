 ...
 * 
 */
package caf.war.ADESPCommonTaskImpl.objectionstaskview;
...
@ManagedBean(name = "ObjectionsTaskViewDefaultxhtmlView")
@SessionScoped
@ExpireWithPageFlow
@DTManagedBean(displayName = "ObjectionsTaskView/default", beanType = BeanType.PAGE)
public class ObjectionsTaskViewDefaultxhtmlView extends com.webmethods.caf.faces.bean.task.BaseTaskDetailsPageBean {
	...
	private static final String[][] TASKDISPLAYPROVIDER_PROPERTY_BINDINGS = new String[][] {
		{"#{TaskDisplayProvider.taskID}", "#{ObjectionsTaskView.taskID}"},
	};
	...
	public boolean getHasVotingDetailsPermission() {
		return getObjectionsTask().getCanUserViewTaskVotingStatistics();
	}
	
	public List<TaskVoteEntry> getListOfVotingTableEntries() {
		List<TaskVoteEntry> listOfVotingTableEntries = null;
		try {
			listOfVotingTableEntries = TaskVotingServiceFactory.getVotingService().getTaskVotesDetails(Integer.valueOf(getObjectionsTask().getTaskID()));
		} catch (Exception e) {
            this.error(e);
            this.log(e);
        }
	...
	public String getTaskVotingStrategy() {
        final TaskVotingTable table = TaskVotingServiceFactory.getVotingService().parseVotingTable(getObjectionsTask().getTaskInfo().getTaskVotingTable());
	 ...
	 */
	 public int getTotalUserAssignedCount() {
        ...
        try {
            totalUserAssignedCount = TaskVotingServiceFactory.getVotingService().getTotalUserAssignedCount(Integer.valueOf(getObjectionsTask().getTaskID()));
        }
        catch (Exception e) {
            this.error(e);
            this.log(e);
    ...
    public Map<String, Integer> getTaskVotingCount() {
        ...
        try {
            map = (Map<String, Integer>)TaskVotingServiceFactory.getVotingService().getTaskVotesCountByStatus(Integer.valueOf(getObjectionsTask().getTaskID()));
        }
        catch (Exception e) {
            this.error(e);
            this.log(e);
	...
	public String cancelView() {
		try {
			...
			String url = getObjectionsTaskView().getFinishUrl();
	...
	public String submitTask() {
		try {
			...
			// do the work
			getObjectionsTask().applyChanges();
		} catch (Exception e) {
			error(e);
			log(e);
		}
	...
	public String assignToPrincipals() {
		try {
			...
			TaskContentProvider tp = getObjectionsTask();
	...
	private static final String[][] INITIALIZE_PROPERTY_BINDINGS = new String[][] {
		{"#{ObjectionsTask.reset}", null}
	};
	private transient caf.war.ADESPCommonTaskImpl.objectionstaskview.ObjectionsTaskView objectionsTaskView = null;
	...
	private static final String[][] OBJECTIONSTASK_PROPERTY_BINDINGS = new String[][] {
		{"#{ObjectionsTask.taskID}", "#{ObjectionsTaskView.taskID}"},
		{"#{ObjectionsTask.autoAccept}", "false"},
		{"#{ObjectionsTask.adhocRouting}", "false"},
	};
	...
	public caf.war.ADESPCommonTaskImpl.objectionstaskview.ObjectionsTaskView getObjectionsTaskView()  {
		if (objectionsTaskView == null) {
		    objectionsTaskView = (caf.war.ADESPCommonTaskImpl.objectionstaskview.ObjectionsTaskView)resolveExpression("#{ObjectionsTaskView}");
		}
		return objectionsTaskView;
	}
