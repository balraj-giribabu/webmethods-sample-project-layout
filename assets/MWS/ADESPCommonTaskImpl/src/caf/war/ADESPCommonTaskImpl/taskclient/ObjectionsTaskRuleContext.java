...
import caf.war.ADESPCommonTaskImpl.is.document.ADESPCommon_documents_objections_TaskConfig;
...
@ManagedBean(name = "ObjectionsTaskRuleContext")
@SessionScoped
@DTManagedBean(displayName = "ObjectionsTask Rule Context", beanType = BeanType.DEFAULT)
public class ObjectionsTaskRuleContext  extends  com.webmethods.caf.faces.data.task.impl.BaseTaskRuleContext {
	...
	public String onQueue() throws Exception{
		ObjectionsTask currentTask = (ObjectionsTask) this.getCurrentTask();
		caf.war.ADESPCommonTaskImpl.taskclient.ObjectionsTask.TaskData taskData = currentTask
				.getTaskData();
		...
		ADESPCommon_documents_objections_BusinessDoc businessDoc = currentTask.getTaskData().getBusinessDoc();
			...
			//		System.out.println("inside parallel task" );
				...
				//		System.out.println("queuing child tasks" );
						...
						for (String assignee : taskConfig.getAssignee()) {
							ObjectionsTask collaborationTask = new ObjectionsTask();
					...
					getQueueTask().getParameters().getQueueTask().getQueueTask().getInput().setParentRequestId(currentTask.getTaskData().getBusinessDoc().getLicenceReqId());
