...
import com.webmethods.caf.faces.data.task.impl.TaskIndexedSearchContentProvider.Task;
import com.webmethods.portal.bizPolicy.command.task.GetTask;
import com.webmethods.portal.bizPolicy.command.task.GetTaskData;
import com.webmethods.portal.bizPolicy.command.task.webservice.SearchTasksWS;

import caf.war.LicenseEligibilityTaskImpl.is.document.ADESPLicenseMgmt_documents_LicenceEligibilityProc_BusinessDoc;
import caf.war.LicenseEligibilityTaskImpl.is.document.IXCommon_documents_TaskConfig;
import caf.war.LicenseEligibilityTaskImpl.is.document.IXCommon_documents_TaskConfig.Actions;

import com.softwareag.is.log.Log;
import com.webmethods.caf.faces.annotations.BeanType;

@ManagedBean(name = "LicenseEligibiltyTaskRuleContext")
@SessionScoped
@DTManagedBean(displayName = "LicenseEligibiltyTask Rule Context", beanType = BeanType.DEFAULT)
public class LicenseEligibiltyTaskRuleContext extends com.webmethods.caf.faces.data.task.impl.BaseTaskRuleContext {

	private java.lang.String isChildTask;
	private transient caf.war.LicenseEligibilityTaskImpl.wsclient.ixtaskmanagement.ws.provider.soap.queuetaskoperations.QueueTask queueTask = null;
	...
	public String onQueue() throws Exception {
		LicenseEligibiltyTask currentTask = (LicenseEligibiltyTask) this.getCurrentTask();
		caf.war.LicenseEligibilityTaskImpl.taskclient.LicenseEligibiltyTask.TaskData taskData = currentTask
				.getTaskData();
		...
		ADESPLicenseMgmt_documents_LicenceEligibilityProc_BusinessDoc businessDoc = currentTask.getTaskData().getBusinessDoc();
			...
			//		System.out.println("inside parallel task" );
				...
				//		System.out.println("queuing child tasks" );
						...
						for (String assignee : taskConfig.getAssignee()) {
							LicenseEligibiltyTask collaborationTask = new LicenseEligibiltyTask();
					...
					for (Actions action : taskConfig.getActions()) {
						if (action.getActionName().equalsIgnoreCase("QUEUED")) {
							...
							currentTask.getTaskData().getBusinessDoc().setNotificationId(action.getNotificationId());
						//	System.out.println("Notification ID :"+ action.getNotificationId());
							currentTask.applyChangesNoAccept();
							break;
						}
					}
	...
	public String onCompletion() {
		...
		caf.war.LicenseEligibilityTaskImpl.taskclient.LicenseEligibiltyTask.TaskData taskData = currentTask
				.getTaskData();
		ADESPLicenseMgmt_documents_LicenceEligibilityProc_BusinessDoc businessDoc = currentTask.getTaskData()
				.getBusinessDoc();
		if (currentTask.getTaskInfo().getParentTaskID() == null
				|| currentTask.getTaskInfo().getParentTaskID().isEmpty()) {
	...
	public caf.war.LicenseEligibilityTaskImpl.wsclient.ixtaskmanagement.ws.provider.soap.queuetaskoperations.QueueTask getQueueTask()  {
		if (queueTask == null) {
		    queueTask = (caf.war.LicenseEligibilityTaskImpl.wsclient.ixtaskmanagement.ws.provider.soap.queuetaskoperations.QueueTask)resolveExpression("#{QueueTask}");
		}
