...
import caf.war.PDSRSSubmissionTaskImpl.is.document.IXCommon_documents_TaskConfig;
import caf.war.PDSRSSubmissionTaskImpl.is.document.IXCommon_documents_TaskConfig.Actions;
import caf.war.PDSRSSubmissionTaskImpl.is.document.PDSRSSubmission_documents_BusinessDoc;


import com.webmethods.caf.faces.annotations.BeanType;

@ManagedBean(name = "Pdsrssubmissiontaskrulecontext")
@SessionScoped
@DTManagedBean(displayName = "PDSRSSubmissionTask Rule Context", beanType = BeanType.DEFAULT)
public class Pdsrssubmissiontaskrulecontext  extends  com.webmethods.caf.faces.data.task.impl.BaseTaskRuleContext {

	private static final String[][] ONQUEUE_PROPERTY_BINDINGS = new String[][] {
	};
	private transient caf.war.PDSRSSubmissionTaskImpl.wsclient.ixtaskmanagement.ws.provider.soap.queuetaskoperations.QueueTask queueTask = null;
	...
	public String onQueue()throws Exception {
		Pdsrssubmissiontask currentTask = (Pdsrssubmissiontask) this.getCurrentTask();
		caf.war.PDSRSSubmissionTaskImpl.taskclient.Pdsrssubmissiontask.TaskData taskData = currentTask
				.getTaskData();
		...
		PDSRSSubmission_documents_BusinessDoc businessDoc = currentTask.getTaskData().getBusinessDoc();
			...
			//		System.out.println("inside parallel task" );
				...
				//		System.out.println("queuing child tasks" );
						...
						for (String assignee : taskConfig.getAssignee()) {
							Pdsrssubmissiontask collaborationTask = new Pdsrssubmissiontask();
	...
	public caf.war.PDSRSSubmissionTaskImpl.wsclient.ixtaskmanagement.ws.provider.soap.queuetaskoperations.QueueTask getQueueTask()  {
		if (queueTask == null) {
		    queueTask = (caf.war.PDSRSSubmissionTaskImpl.wsclient.ixtaskmanagement.ws.provider.soap.queuetaskoperations.QueueTask)resolveExpression("#{QueueTask}");
		}
